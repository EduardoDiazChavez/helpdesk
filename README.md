...
- Prisma helpers: `npm run db:push`, `npm run db:seed`, `npm run prisma:generate`, `docker compose -f docker-compose.dev.yml run --rm -p 5555:5555 helpdesk   sh -c "DATABASE_URL=file:/app/prisma/prisma/dev.db npx prisma studio --hostname 0.0.0.0 --port 5555"`
